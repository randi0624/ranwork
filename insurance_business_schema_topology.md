# insurance_business 表结构拓扑

生成时间：2026-06-04 14:26:04  
数据源：Doris MySQL 协议 `192.168.100.114:9030/insurance_business`  
说明：Doris 元数据中未发现显式外键，本文件中的关联关系主要根据同名业务编号字段、关联表命名和字段注释推断；未查询业务数据。

## 1. 分域拓扑图

```mermaid
flowchart LR
  subgraph _____["车辆主数据"]
    vehicle_basic_info["vehicle_basic_info"]
    vehicle_certificate_info["vehicle_certificate_info"]
    vehicle_auth_info["vehicle_auth_info"]
    vehicle_abnormal_info["vehicle_abnormal_info"]
    vehicle_approved_info["vehicle_approved_info"]
    vehicle_bank_info["vehicle_bank_info"]
    vehicle_team_info["vehicle_team_info"]
  end
  subgraph ____["车型车款"]
    manufacturer_company_info["manufacturer_company_info"]
    brand_info["brand_info"]
    series_info["series_info"]
    model_info["model_info"]
    pattern_info["pattern_info"]
    pattern_battery_info["pattern_battery_info"]
    pattern_certificate_info["pattern_certificate_info"]
    pattern_drive_electric_motor_info["pattern_drive_electric_motor_info"]
    pattern_assist_drive_function_info["pattern_assist_drive_function_info"]
  end
  subgraph ____["保险业务"]
    insurance_company_info["insurance_company_info"]
    insurance_type_info["insurance_type_info"]
    vehicle_insurance_info["vehicle_insurance_info"]
    vehicle_insurance_subordinate_info["vehicle_insurance_subordinate_info"]
    vehicle_split_insurance_info["vehicle_split_insurance_info"]
    vehicle_insurance_id_info["vehicle_insurance_id_info"]
    type_insurance_association["type_insurance_association"]
    store_insurance_association["store_insurance_association"]
    file_insurance_association["file_insurance_association"]
  end
  subgraph ____["理赔事故"]
    insurance_vehicle_claims_info["insurance_vehicle_claims_info"]
    insurance_vehicle_claims_history["insurance_vehicle_claims_history"]
    vehicle_claim_event["vehicle_claim_event"]
    vehicle_outstanding_loss_estimate["vehicle_outstanding_loss_estimate"]
    accident_responsibility_report["accident_responsibility_report"]
    morality_risk_analyze_report["morality_risk_analyze_report"]
    email_send_info["email_send_info"]
    suspected_is_delete["suspected_is_delete"]
  end
  subgraph ____["金融门店"]
    bank_info["bank_info"]
    store_info["store_info"]
    loan_product_info["loan_product_info"]
    vehicle_loan_event["vehicle_loan_event"]
    credit_company_info["credit_company_info"]
  end
  subgraph ______["标签关注策略"]
    attention_info["attention_info"]
    attention_basic_info["attention_basic_info"]
    attention_big_data_info["attention_big_data_info"]
    attention_tag_association["attention_tag_association"]
    tag_type["tag_type"]
    tag_indicators["tag_indicators"]
    tag_content["tag_content"]
    vehicle_tag["vehicle_tag"]
    strategy_risk_insight["strategy_risk_insight"]
    strategy_suggestion["strategy_suggestion"]
    insight_suggestion_association["insight_suggestion_association"]
    vehicle_risk_insight["vehicle_risk_insight"]
    risk_type_association_info["risk_type_association_info"]
    strategy_prompt["strategy_prompt"]
  end
  subgraph _______["文件零配件报告"]
    file_info["file_info"]
    file_association["file_association"]
    import_history_info["import_history_info"]
    report_center_info["report_center_info"]
    part_type_info["part_type_info"]
    spare_parts_info["spare_parts_info"]
    part_type_association["part_type_association"]
  end
  accident_responsibility_report -->|report_code| insurance_vehicle_claims_info
  accident_responsibility_report -->|vehicle_code| vehicle_basic_info
  attention_basic_info -->|attention_id| attention_info
  attention_basic_info -->|brand_id| brand_info
  attention_basic_info -->|model_id| model_info
  attention_basic_info -->|pattern_id| pattern_info
  attention_basic_info -->|series_id| series_info
  attention_big_data_info -->|attention_id| attention_info
  attention_tag_association -->|attention_id| attention_info
  attention_tag_association -->|tag_id| tag_content
  brand_info -->|manufacturer_id| manufacturer_company_info
  email_send_info -->|cw_id| suspected_is_delete
  email_send_info -->|vehicle_code| vehicle_basic_info
  file_association -->|file_id| file_info
  file_insurance_association -->|file_id| file_info
  file_insurance_association -->|insurance_id| vehicle_insurance_info
  insight_suggestion_association -->|risk_alarm_id| strategy_risk_insight
  insight_suggestion_association -->|suggestion_id| strategy_suggestion
  insurance_vehicle_claims_history -->|claim_code| insurance_vehicle_claims_info
  insurance_vehicle_claims_info -->|vehicle_code| vehicle_basic_info
  model_info -->|series_id| series_info
  pattern_assist_drive_function_info -->|pattern_id| pattern_info
  pattern_battery_info -->|pattern_id| pattern_info
  pattern_certificate_info -->|pattern_id| pattern_info
  pattern_drive_electric_motor_info -->|pattern_id| pattern_info
  pattern_info -->|model_id| model_info
  report_center_info -->|claim_code| insurance_vehicle_claims_info
  report_center_info -->|report_code| insurance_vehicle_claims_info
  report_center_info -->|vehicle_code| vehicle_basic_info
  series_info -->|brand_id| brand_info
  spare_parts_info -->|brand_id| brand_info
  spare_parts_info -->|series_id| series_info
  spare_parts_info -->|insurance_id| vehicle_insurance_info
  store_info -->|manufacturer_id| manufacturer_company_info
  store_insurance_association -->|store_id| store_info
  store_insurance_association -->|insurance_id| vehicle_insurance_info
  type_insurance_association -->|insurance_type_id| insurance_type_info
  type_insurance_association -->|insurance_id| vehicle_insurance_info
  vehicle_abnormal_info -->|vehicle_code| vehicle_basic_info
  vehicle_approved_info -->|vehicle_code| vehicle_basic_info
  vehicle_auth_info -->|manufacturer_id| manufacturer_company_info
  vehicle_auth_info -->|vehicle_code| vehicle_basic_info
  vehicle_bank_info -->|bank_id| bank_info
  vehicle_bank_info -->|product_type_id| loan_product_info
  vehicle_bank_info -->|store_id| store_info
  vehicle_bank_info -->|vehicle_code| vehicle_basic_info
  vehicle_basic_info -->|brand_id| brand_info
  vehicle_basic_info -->|model_id| model_info
  vehicle_basic_info -->|pattern_id| pattern_info
  vehicle_basic_info -->|series_id| series_info
  vehicle_basic_info -->|store_id| store_info
  vehicle_certificate_info -->|vehicle_code| vehicle_basic_info
  vehicle_claim_event -->|report_code| insurance_vehicle_claims_info
  vehicle_claim_event -->|vehicle_code| vehicle_basic_info
  vehicle_insurance_id_info -->|vehicle_code| vehicle_basic_info
  vehicle_insurance_id_info -->|insurance_id| vehicle_insurance_info
  vehicle_insurance_info -->|insurance_company_id| insurance_company_info
  vehicle_insurance_info -->|vehicle_code| vehicle_basic_info
  vehicle_insurance_info_backup -->|insurance_company_id| insurance_company_info
  vehicle_insurance_info_backup -->|vehicle_code| vehicle_basic_info
  vehicle_insurance_info_backup -->|insurance_id| vehicle_insurance_info
  vehicle_insurance_info_backup -->|vehicle_insurance_id| vehicle_insurance_info
  vehicle_insurance_info_backup_copy -->|insurance_company_id| insurance_company_info
  vehicle_insurance_info_backup_copy -->|insurance_type_id| insurance_type_info
  vehicle_insurance_info_backup_copy -->|vehicle_code| vehicle_basic_info
  vehicle_insurance_info_backup_copy -->|insurance_id| vehicle_insurance_info
  vehicle_insurance_info_backup_copy -->|vehicle_insurance_id| vehicle_insurance_info
  vehicle_insurance_subordinate_info -->|insurance_type_id| insurance_type_info
  vehicle_insurance_subordinate_info -->|vehicle_code| vehicle_basic_info
  vehicle_insurance_subordinate_info -->|insurance_id| vehicle_insurance_info
  vehicle_insurance_subordinate_info -->|vehicle_insurance_id| vehicle_insurance_info
  vehicle_insurance_subordinate_info_backup -->|insurance_type_id| insurance_type_info
  vehicle_insurance_subordinate_info_backup -->|vehicle_code| vehicle_basic_info
  vehicle_insurance_subordinate_info_backup -->|insurance_id| vehicle_insurance_info
  vehicle_insurance_subordinate_info_backup -->|vehicle_insurance_id| vehicle_insurance_info
  vehicle_loan_event -->|bank_id| bank_info
  vehicle_loan_event -->|product_type_id| loan_product_info
  vehicle_loan_event -->|store_id| store_info
  vehicle_loan_event -->|vehicle_code| vehicle_basic_info
  vehicle_outstanding_loss_estimate -->|insurance_company_id| insurance_company_info
  vehicle_outstanding_loss_estimate -->|report_code| insurance_vehicle_claims_info
  vehicle_outstanding_loss_estimate -->|vehicle_code| vehicle_basic_info
  vehicle_repair_event -->|vehicle_code| vehicle_basic_info
  vehicle_risk_insight -->|risk_alarm_id| strategy_risk_insight
  vehicle_risk_insight -->|vehicle_code| vehicle_basic_info
  vehicle_split_insurance_info -->|insurance_id| vehicle_insurance_info
  vehicle_tag -->|tag_id| tag_content
  vehicle_tag -->|vehicle_code| vehicle_basic_info
```

## 2. 主要关联关系

| 来源表 | 关联字段 | 目标表 | 字段注释 | 依据 |
|---|---|---|---|---|
| accident_responsibility_report | `report_code` | insurance_vehicle_claims_info | 报案号 | 命名推断 |
| accident_responsibility_report | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| attention_basic_info | `attention_id` | attention_info | 关注编号 | 命名推断 |
| attention_basic_info | `brand_id` | brand_info | 品牌编号 | 命名推断 |
| attention_basic_info | `model_id` | model_info | 车型编号 | 命名推断 |
| attention_basic_info | `pattern_id` | pattern_info | 车款编号 | 命名推断 |
| attention_basic_info | `series_id` | series_info | 车系编号 | 命名推断 |
| attention_big_data_info | `attention_id` | attention_info | 关注编号 | 命名推断 |
| attention_tag_association | `attention_id` | attention_info | 关注编号 | 命名推断 |
| attention_tag_association | `tag_id` | tag_content | 标签编号 | 命名推断 |
| brand_info | `manufacturer_id` | manufacturer_company_info | 制造企业编号 | 命名推断 |
| email_send_info | `cw_id` | suspected_is_delete | 疑似事故编号 | 命名推断 |
| email_send_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| file_association | `file_id` | file_info | 文件编号 | 命名推断 |
| file_insurance_association | `file_id` | file_info | 文件编号 | 命名推断 |
| file_insurance_association | `insurance_id` | vehicle_insurance_info | 保险编号 | 命名推断 |
| insight_suggestion_association | `risk_alarm_id` | strategy_risk_insight | 洞察编号 | 命名推断 |
| insight_suggestion_association | `suggestion_id` | strategy_suggestion | 建议编号 | 命名推断 |
| insurance_vehicle_claims_history | `claim_code` | insurance_vehicle_claims_info | 理赔编号 | 命名推断 |
| insurance_vehicle_claims_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| model_info | `series_id` | series_info | 车系编号 | 命名推断 |
| morality_risk_analyze_report | `accident_code` | accident_responsibility_report | 事故责任报告编号 | 同名字段 |
| part_type_info | `type_id` | part_type_association | 分类编号 | 命名推断 |
| pattern_assist_drive_function_info | `pattern_id` | pattern_info | 车款编号 | 命名推断 |
| pattern_battery_info | `pattern_id` | pattern_info | 车款编号 | 命名推断 |
| pattern_certificate_info | `pattern_id` | pattern_info | 车款编号 | 命名推断 |
| pattern_drive_electric_motor_info | `pattern_id` | pattern_info | 车款编号 | 命名推断 |
| pattern_info | `model_id` | model_info | 车型编号 | 命名推断 |
| report_center_info | `claim_code` | insurance_vehicle_claims_info | 理赔编号 | 命名推断 |
| report_center_info | `report_code` | insurance_vehicle_claims_info | 报案号 | 命名推断 |
| report_center_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| series_info | `brand_id` | brand_info | 品牌编号 | 命名推断 |
| spare_parts_info | `brand_id` | brand_info | 品牌编号 | 命名推断 |
| spare_parts_info | `part_code` | part_type_association | 配件编号 | 命名推断 |
| spare_parts_info | `type_id` | part_type_association | 配件类型 | 命名推断 |
| spare_parts_info | `series_id` | series_info | 车系编号 | 命名推断 |
| spare_parts_info | `insurance_id` | vehicle_insurance_info | 保险公司 | 命名推断 |
| store_info | `manufacturer_id` | manufacturer_company_info | 制造企业编号 | 命名推断 |
| store_insurance_association | `store_id` | store_info | 门店编号 | 命名推断 |
| store_insurance_association | `insurance_id` | vehicle_insurance_info | 保险公司编号 | 命名推断 |
| strategy_risk_insight | `prompt_id` | strategy_prompt | 提示语编号 | 同名字段 |
| tag_type | `type_id` | part_type_association | 业务编号 | 命名推断 |
| type_insurance_association | `insurance_type_id` | insurance_type_info | 险种编号 | 命名推断 |
| type_insurance_association | `insurance_id` | vehicle_insurance_info | 保险公司编号 | 命名推断 |
| vehicle_abnormal_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_approved_info | `vehicle_code` | vehicle_basic_info | 车辆编号，主键 | 命名推断 |
| vehicle_auth_info | `credit_company_id` | credit_company_info | 征信公司编号，数据接收方 | 同名字段 |
| vehicle_auth_info | `manufacturer_id` | manufacturer_company_info | 车辆制造商，数据提供方 | 命名推断 |
| vehicle_auth_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_bank_info | `bank_id` | bank_info | 银行编号 | 命名推断 |
| vehicle_bank_info | `product_type_id` | loan_product_info | 贷款产品编号 | 命名推断 |
| vehicle_bank_info | `store_id` | store_info | 门店编号 | 命名推断 |
| vehicle_bank_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_basic_info | `brand_id` | brand_info | 品牌编号 | 命名推断 |
| vehicle_basic_info | `model_id` | model_info | 车型编号 | 命名推断 |
| vehicle_basic_info | `pattern_id` | pattern_info | 车款编号 | 命名推断 |
| vehicle_basic_info | `series_id` | series_info | 车系编号 | 命名推断 |
| vehicle_basic_info | `store_id` | store_info | 门店编号 | 命名推断 |
| vehicle_certificate_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_claim_event | `report_code` | insurance_vehicle_claims_info | 报案号 | 命名推断 |
| vehicle_claim_event | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_insurance_id_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_insurance_id_info | `insurance_id` | vehicle_insurance_info | 保险编号 | 命名推断 |
| vehicle_insurance_info | `insurance_company_id` | insurance_company_info | 承保机构 | 命名推断 |
| vehicle_insurance_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_insurance_info_backup | `insurance_company_id` | insurance_company_info | 承保机构 | 命名推断 |
| vehicle_insurance_info_backup | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_insurance_info_backup | `insurance_id` | vehicle_insurance_info | 保单编号 | 命名推断 |
| vehicle_insurance_info_backup | `vehicle_insurance_id` | vehicle_insurance_info | 车辆保险编号 | 命名推断 |
| vehicle_insurance_info_backup_copy | `insurance_company_id` | insurance_company_info | 承保机构 | 命名推断 |
| vehicle_insurance_info_backup_copy | `insurance_type_id` | insurance_type_info | 险种编号 | 命名推断 |
| vehicle_insurance_info_backup_copy | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_insurance_info_backup_copy | `system_insurance_id` | vehicle_insurance_id_info | 系统保单编号 | 同名字段 |
| vehicle_insurance_info_backup_copy | `insurance_id` | vehicle_insurance_info | 保单编号 | 命名推断 |
| vehicle_insurance_info_backup_copy | `vehicle_insurance_id` | vehicle_insurance_info | 车辆保险编号 | 命名推断 |
| vehicle_insurance_subordinate_info | `insurance_type_id` | insurance_type_info | 险种编号 | 命名推断 |
| vehicle_insurance_subordinate_info | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_insurance_subordinate_info | `insurance_id` | vehicle_insurance_info | 保单编号 | 命名推断 |
| vehicle_insurance_subordinate_info | `vehicle_insurance_id` | vehicle_insurance_info | 车辆保险编号 | 命名推断 |
| vehicle_insurance_subordinate_info_backup | `insurance_type_id` | insurance_type_info | 险种编号 | 命名推断 |
| vehicle_insurance_subordinate_info_backup | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_insurance_subordinate_info_backup | `insurance_id` | vehicle_insurance_info | 保单编号 | 命名推断 |
| vehicle_insurance_subordinate_info_backup | `vehicle_insurance_id` | vehicle_insurance_info | 车辆保险编号 | 命名推断 |
| vehicle_loan_event | `bank_id` | bank_info | 银行编号 | 命名推断 |
| vehicle_loan_event | `product_type_id` | loan_product_info | 贷款产品编号 | 命名推断 |
| vehicle_loan_event | `store_id` | store_info | 门店编号 | 命名推断 |
| vehicle_loan_event | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_outstanding_loss_estimate | `insurance_company_id` | insurance_company_info | 保险公司编号 | 命名推断 |
| vehicle_outstanding_loss_estimate | `report_code` | insurance_vehicle_claims_info | 报案号 | 命名推断 |
| vehicle_outstanding_loss_estimate | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_repair_event | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_risk_insight | `risk_alarm_id` | strategy_risk_insight | 风险洞察编号 | 命名推断 |
| vehicle_risk_insight | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |
| vehicle_split_insurance_info | `insurance_id` | vehicle_insurance_info | 保单编号 | 命名推断 |
| vehicle_tag | `tag_id` | tag_content | 标签编号 | 命名推断 |
| vehicle_tag | `vehicle_code` | vehicle_basic_info | 车辆编号 | 命名推断 |

## 3. 表清单

| 表名 | 表注释 | 字段数 |
|---|---|---:|
| accident_responsibility_report | 事故责任评定报告表 | 13 |
| attention_basic_info | 高级筛选基础数据信息表 | 28 |
| attention_big_data_info | 高级筛选数仓信息表 | 20 |
| attention_info | 高级筛选关注信息表 | 12 |
| attention_tag_association | 高级筛选关注与标签关系表 | 3 |
| bank_info | 银行信息表 | 24 |
| brand_info | 品牌信息表 | 12 |
| credit_company_info | 征信公司信息表 | 20 |
| email_send_info | 邮件发送信息表 | 15 |
| file_association | 文件信息关联表 | 4 |
| file_info | 文件信息表 | 15 |
| file_insurance_association | 保险文件信息关联表 | 3 |
| import_history_info | 文件导入时间信息表 | 17 |
| insight_suggestion_association | 洞察与建议关系表 | 3 |
| insurance_company_info | 保险公司信息表 | 30 |
| insurance_type_info | 险种信息表 | 14 |
| insurance_vehicle_claims_history | 车辆理赔调查记录表 | 11 |
| insurance_vehicle_claims_info | 车辆理赔信息表 | 32 |
| loan_product_info | 贷款产品信息表 | 13 |
| manufacturer_company_info | 制造企业信息表 | 20 |
| model_info | 车型信息表 | 12 |
| morality_risk_analyze_report | 道德风险研判报告表 | 12 |
| part_type_association | 零件分类信息关联表 | 3 |
| part_type_info | 零配件分类信息表 | 11 |
| pattern_assist_drive_function_info | 辅助驾驶功能信息表 | 23 |
| pattern_battery_info | 车款电池信息表 | 23 |
| pattern_certificate_info | 车款合格证信息表 | 42 |
| pattern_drive_electric_motor_info | 车款电池信息表 | 18 |
| pattern_info | 车款信息表 | 13 |
| report_center_info | 数据报告信息表 | 19 |
| risk_type_association_info | 风险类型信息表 | 6 |
| series_info | 车系信息表 | 12 |
| spare_parts_info | 零配件表 | 19 |
| store_info | 门店信息表 | 22 |
| store_insurance_association | 保险公司 - 门店关联表 | 3 |
| strategy_prompt | 提示语信息表 | 11 |
| strategy_risk_insight | 风险洞察策略表 | 20 |
| strategy_suggestion | 策略行动建议表 | 13 |
| suspected_is_delete | 删除的疑似事故编号表 | 2 |
| tag_content | 标签内容表 | 18 |
| tag_indicators | 标签指标库表 | 17 |
| tag_type | 标签分类表 | 15 |
| type_insurance_association | 保险公司 - 险种关联表 | 3 |
| vehicle_abnormal_info | 行驶异常车辆信息表 | 2 |
| vehicle_approved_info | 车辆核保信息表 | 3 |
| vehicle_auth_info | 车辆授权信息表 | 17 |
| vehicle_bank_info | 车辆银行信息表 | 28 |
| vehicle_basic_info | 车辆基础信息表 | 40 |
| vehicle_certificate_info | 车辆合格证信息表 | 18 |
| vehicle_claim_event | 车辆理赔信息表 | 24 |
| vehicle_insurance_id_info | 保险编号信息表 | 7 |
| vehicle_insurance_info | 保险公司信息复制表 | 25 |
| vehicle_insurance_info_backup | 保险信息表 | 24 |
| vehicle_insurance_info_backup_copy | 保险信息表 | 28 |
| vehicle_insurance_subordinate_info | 保险信息险种子表表 | 17 |
| vehicle_insurance_subordinate_info_backup | 保险信息险种子表表 | 17 |
| vehicle_loan_event | 车辆贷款信息表 | 24 |
| vehicle_outstanding_loss_estimate | 车辆未决估损信息表 | 34 |
| vehicle_repair_event | 车辆维保事件表 | 15 |
| vehicle_risk_insight |  | 3 |
| vehicle_split_insurance_info | 保险车辆拆分信息表 | 15 |
| vehicle_tag |  | 3 |
| vehicle_team_info | 车队详情表 | 13 |
